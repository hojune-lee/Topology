...
\begin{thm}{Finite Intersection Property Criterion}{finite intersection property criterion}
    ...
    i.e., $\bigcap_{C \in \CC} C \neq \emptyset$. 
\end{thm}

\begin{proof}
    First, it is important to observe the connection between definition of open covering 
    and finite intersection property of closed sets. 
    Let $\CC$ be a given collection of closed sets in $X$. Then, we can construct 
    \[\CA = \{X \setminus C \mid C \in \CC\}\]
    This is collection of open sets in $X$. Suppose that $\CA$ has a finite subcovering of $X$. 
    \[\{A_1, \cdots, A_n\} \subseteq \CA \implies \bigcup_{i \in [n]} A_i = X\]
    This implies that 
    \[\bigcap_{i \in [n]} C_i = \emptyset \text{ where $C_i = X \setminus A_i \in \CC$ }\]
    However, $X$ is compact if and only if for any open covering $\CA$, we can find finite subcovering in $\CA$. 
    So, $X$ is compact if and only if any collection of open sets $\CA$ doesn't have finite subcovering, then $\CA$ not cover $X$. 
    Latter can be translated into words of $\CC$ and finite intersection property: 
    \begin{center}
        \textit{Any collection $\CC$ of closed subsets in $X$ having finite intersection property.}
    \end{center}
\end{proof}
...
\begin{proof}
    Let $\CA$ be any open covering of $X$. 
    If $X \in \CA$, then statement automatically holds for any $\delta > 0$. 
    So suppose that $X \notin \CA$. 
    Since $X$ is compact, we can find finite subcovering $\{A_1, \cdots, A_n\} \subseteq \CA$. 
    Denote $C_i = X \setminus A_i$ for each $i \in [n]$. Define 
    \[f(x) = \frac{1}{n}\sum_{i = 1}^{n} d(x, C_i)\]
    Then \hl{$f(x) > 0$ (strictly larger than 0) since $x$ must contained in some open cover ($= A_i$)}, so 
    \[x \in A_i \implies B_d(x, \varepsilon) \subseteq A_i \implies d(x, C_i) > \varepsilon \implies f(x) > 0\]
    However, according to \ref{rem:12.4} and summation of continuous functions are continuous, $f$ is continuous over $X$. 
    Since $X$ is compact, $f(X)$ also compact by \ref{thm:continuous preserves compact} 
    and so closed and bounded. We can find minimum($=\delta$) of $f(X)$. 
    This $\delta > 0$ become a Lebesgue number we claimed. Suppose that $B \subseteq X$ and diameter of $B$ $< \delta$. 
    Let $x_0 \in B$ then 
    \[\delta \leq f(x_0) \leq d(x_0, C_m) \text{ where $C_m$ is the most far $C_i$}\]
    Implies that $B(f(x_0), \delta) \subseteq A_m \in \CA$. 
\end{proof}
...
\begin{defn}{Sequentially Compact}{sequentially compact}
    ...
    $X$ is said to be \textit{sequentially compact} if every subsequence of points of $X$ has a 
...
\begin{proof}
    Suppose that $X$ is locally compact Hausdorff space. 
    Let $\infty \notin X$ be the newly introduced object. Let's construct 
    \[Y = X \cup \{\infty\}\]
    And we give following topology for $Y$. 
    \[\CT_Y = \CT_X \cup \{Y \setminus C \mid C \text{ is compact in $X$ }\}\]
    This satisfies the axiom of topology: 
    \begin{enumerate}
        \item $\emptyset \in \CT_X$, $Y = Y \setminus \emptyset \in \{Y \setminus C\}$
        \item 
        \begin{itemize}
            \item For $U_1, U_2 \in \CT_X, U_1 \cap U_2 \in \CT_X$. 
            \item For $U_1 \in \CT_X, U_2 = Y \setminus C$, since $U_1 \subseteq X$, $U_1 \cap U_2 = U_1 \cap (X \setminus C)$. 
            Since $X$ is Hausdorff, compact subset $C$ is closed so $X \setminus C$ is open. It implies that 
            $U_1 \cap U_2 = U_1 \cap (X \setminus C) \in \CT_X$. 
            \item For $U_1 = Y \setminus C_1, U_2 = Y \setminus C_2$, we obtain $U_1 \cap U_2 = Y \setminus (C_1 \cup C_2)$. 
            Then by \ref{lem:compact union intersection}, $C_1 \cup C_2$ is compact in $X$ and so $U_1 \cap U_2 \in \{Y \setminus C\}$. 
        \end{itemize}
        \item 
        \begin{itemize}
            \item Arbitrary union in $\CT_X$ is in $\CT_X$. 
            \item $\bigcup (Y \setminus C_\beta) = Y \setminus (\bigcap C_\beta)$, however by \ref{lem:compact union intersection}, in Hausdorff space arbitrary intersection of compact set is compact. So, it is in $\{Y \setminus C\}$. 
            \item $\left(\bigcup U_\alpha \right) \cup \left(\bigcup Y \setminus C_\beta\right) = U \cup (Y \setminus C) = Y \setminus (C \setminus U)$. However, $C \setminus U$ is closed 
            subset of compact set $C$ so $C \setminus U$ is compact, it is in $\{Y \setminus C\}$. 
        \end{itemize}
    \end{enumerate}
    Then remaining things to do is showing that: (1) $X$ is subspace of $Y$. (2) $Y$ is compact. (3) $Y$ is Hausdorff. 
    
    (1) If $U \in \CT_X$, then $U \cap X = U \in \CT_X$. If $U = Y \setminus C$, $U \cap X = X \setminus C \in \CT_X$ since $C$ is closed. 
    And since $\CT_X \subseteq \CT_Y$, $X$ is subspace of $Y$. 

    (2) Let $\CA$ be an open covering of $Y$. To cover $\infty \in Y$, $\CA$ must contain at least one 
    open set of type $A = Y \setminus C$. Then, $\CA - A$ is open covering of $C$. Since $C$ is compact in $X$ and by \ref{cor:compact is absolute} it also compact in $Y$. 
    So there are finite subcovering $\{A_n\}$ from $\CA - A$, then $\{A_n\} \cup A$ becomes finite subcovering of $\CA$ of $Y$. 

    (3) If $x, y \in X \subseteq Y$, then we can seperate them via open sets. 
    Without loss of generality, let $x \in X, y = \infty$. 
    Since $X$ is locally compact, we can construct neighborhood $U$ of $x$ containing in $C$. 
    Then, $y \in Y \setminus C$ which is open in $Y$. $U$ and $Y \setminus C$ is open disjoint neighborhood of $x, y$ respectively in $Y$. 

    Now we prove converse. Suppose that $Y$ satisfies conditions in the state. 
    Then $X$ is subspace of Hausdorff, so it also become Hausdorff. 
    Let $x \in X$. Since $Y \setminus X = \{\infty\}$ by second condition and $Y$ is Hausdorff, 
    in Hausdorff space $Y$, we can find disjoint neighborhood $U, V$ of $x, \infty$ respectively. 
    Then $Y \setminus V$ is closed in $Y$ and contained in $X$. Since $Y$ is compact, 
    $Y \setminus V$ is compact by \ref{thm:closed in compact is compact}. 
    By \ref{lem:subspace compactness}, it is a compact in $X$ and containing $U$. 
    So, $X$ is locally compact. 

    Uniqueness of embedding is obvious. 
\end{proof}
...
\begin{proof}
    ...
    Means that $x \in U \subseteq X \setminus V$. Since $X \setminus V$ is closed, 
    \[x \in U \subseteq \overbar{U} \subseteq W\]
...
\begin{proof}
    Let $X$ be compact Hausdorff space and $A, B$ are two disjoint closed subset in $X$. 
    Suppose that we choose any point $\alpha \in A$. For every point $\beta \in B$, since $X$ is Hausdorff, 
    we can find $U_\alpha^\beta, V_\beta$ such that $\alpha \in U_\alpha^\beta$ and $\beta \in V_\beta$. 
    Collection of $V_\beta$ such that $\{V_\beta \mid \beta \in B\}$ forms an open covering of $B$. 
    Since $B$ is closed subset in compact space, $B$ also compact by \ref{thm:closed in compact is compact}. 
    So we can find $\{V_{\beta_1}, \cdots, V_{\beta_n}\}$ a finite subcovering. 
    Then we have corresponding $U_\alpha^{\beta_i}$ an open neighborhood of $\alpha$. 
    Then, 
    \[U_\alpha^* = \bigcap U_\alpha^{\beta_i}, V^*_\alpha = \bigcup V_{\beta_i} \implies U^*_\alpha \cap V^*_\alpha = \emptyset, \text{ both are open }, \alpha \in U^*_\alpha, \quad B \subseteq V^*_\alpha\]
    Do this all for $\alpha \in A$, obtaining $\{U^*_\alpha \mid \alpha \in A\}$ forms open covering of $A$. 
    By the similar reasoning, we finally obtain finite subcovering $\{U^*_{\alpha_i}\}_{i = 1}^{m}$
    \[A \subseteq U = \bigcup U^*_{\alpha_i}, \quad B \subseteq V = \bigcap V^*_{\alpha_i}\]
    Where $U, V$ are disjoint open set in $X$. 
\end{proof}
...
\begin{defn}{First Countable}{first countable}
    A space $X$ is first countable if for any point $x \in X$, there exists countable open neighborhoods 
    $\{U_n\}_{n \in \BN}$ of $x$ such that for arbitrary open set $U$ containing $x$, we can find $U_i$ such that 
    $x \in U_i \subseteq U_n$. 
\end{defn}
...
\begin{proof}
    Let $\CB = \{L_n\}$ be the countable basis of $X$ and $A, B$ are disjoint closed sets in $X$. 
    Since $A, B$ are closed and $X$ is regular, we can find open sets $U, V$ such that 
    \[x \in V \subseteq \overbar{V} \subseteq U \subseteq X \setminus B\]
    By the definition of basis, we can find a basis $L_i \in \CB$ such that $x \in L_i \subseteq V$. 
    Do this for all $x \in A$ and collect $L_i$. Then such collection is subcollection of $\{L_n\}$, so countable and 
    it becomes the \textit{countable subcovering} of $A$ of $\CB$. 
    Denote it by $\{U_n\}$. 
    Do exactly same things for $B$ and construct countable subcovering $\{V_n\}$ of $B$. 
    Now, let $U = \bigcup U_n$ and $V = \bigcup V_n$ then they are open sets containing $A, B$ respectively. 
    Yet, they are not need to be disjoint. 
    Let's newly define 
    \[U_n' = U_n \setminus \bigcup_{i = 1}^{n} \overbar{V_i}, \quad V_n' = V_n \setminus \bigcup_{i = 1}^{n} \overbar{U_i}\]
    Then clearly $U_n', V_n'$ are open, and $\{U_n'\}$ covers $A$, $\{V_n'\}$ covers $B$. 
    Finally, 
    \[U' = \bigcup U'_n , \quad V' = \bigcup V'_n\]
    are disjoint open sets such that seperating $A, B$ in $X$. 
\end{proof}
