...
\begin{proof}
    ...
    Hence, 
    \[p \circ \tilde{f} = f\]
    Finally, we already know that $p$ is open map \ref{prop:property of covering map}. 
    Hence, for any open set $U \subseteq E$, $p(U)$ is also open in $B$. 
    We take its preimage through continuous map $f$, so $f^{-1}(p(U))$ is open in $Y$. However, 
    \[\tilde{f}^{-1}(U) = f^{-1}(p(U))\]
    So, $\tilde{f}^{-1}(U)$ is open in $Y$ hence $\tilde{f}$ is continuous. 
\end{proof}
