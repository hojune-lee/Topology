...
\begin{cor}{}{18.3}
    If $X$ is $T_1$ and Tietze extension property holds then $X$ is normal. 
\end{cor}

\begin{proof}
    Let $A, B \subseteq X$ be disjoint closed subsets. 
    Then, we can naturally construct constant functions(hence continuous)
    \[f_A : A \rightarrow 0, \quad f_B : B \rightarrow 1\]
    Since $A, B$ are disjoint, 
    \[f_{A \cup B} = \begin{cases}
        f_A & \text{ in $A$} \\
        f_B & \text{ in $B$}
    \end{cases}\]
    is also continuous function such that $f_{A \cup B} : A \cup B \rightarrow [0,1]$. 
    Hence, by \ref{thm:Tietze extension theorem}, we can expand this into continuous function
    \[f^* : X \rightarrow [0,1], \quad f^*|_A \equiv 0 , \quad f^*|_B \equiv 1\]
    Then, we can seperate $A, B$ by two disjoint open sets 
    \[U = (f^*)^{-1}([0, 1/2)), \quad V = (f^*)^{-1}((1/2, 1])\]
    Hence $X$ is normal. 
\end{proof}
...
\hl{Up to here, I introduce quotient as \lq fold', and here also.} 
During the studing, I wonder the relation between them and obtain following results. 
Intuitively, quotient map is arbitrary folding and covering map is somewhat \lq uniform' folding. 
Hence quotient map is stronger but it can lose the topological properties, however 
covering map is weaker but it can obtain or preserves many topological properties. 

\begin{rem}{Covering Map is Quotient Map}{covering is quotient}
    Every covering map $p: E \rightarrow B$ is indeed quotient map. 
\end{rem}
...
\begin{proof}
    We use following facts in group theory: 
    \[\textit{Group homomorphism $\varphi$ is injective if and only if $\ker \varphi$ is trivial}\]
    Here, 
    \[\ker{p_*} = \{[\gamma] \mid p: [\gamma] \mapsto [C_{b_0}] \} \subseteq \pi_1(E, e_0)\]
    Hence, suppose that $[p \circ \gamma] = [C_{b_0}]$. Means that $p \circ \gamma$ and $C_{b_0}$ are 
    homotopic relative to end points. So construct 
    \[H: [0,1] \times [0,1] \rightarrow B, \quad F|_{[0,1] \times \{0\}} \equiv C_{b_0}, \quad F|_{[0,1] \times \{1\}} \equiv p \circ \gamma\]
    We can find unique lifting of $C_{b_0}$ whose initial point is $e_0$, indeed it is $C_{e_0}$. 
    Hence, we can lift our homotopy into $E$ by 
    \[\tilde{H}: [0,1] \times [0,1] \rightarrow E, \quad \tilde{H}|_{[0,1] \times \{0\}} \equiv C_{e_0}, \quad \tilde{H}|_{[0,1] \times \{1\}} \equiv \widetilde{p \circ \gamma} = \gamma\]
    The lifting of $\widetilde{p \circ \gamma}$ is uniquely fixed by $\gamma$ since its initial point is $e_0$. 
    Hence, $C_{e_0} \simeq_p \gamma$ and implies that $[\gamma] = [C_{e_0}]$, means that $\ker{p_*} = \{[C_{e_0}]\}$, trivial subgroup.
    Hence, $p_*$ is injective. 
\end{proof}
...
\begin{thm}{The Lifting Criterion}{lifting criterion}
    ...
    \[\text{Exists unique lifting } \tilde{f}: (Y, y_0) \rightarrow (E, e_0) \iff f_*(\pi_1(Y, y_0)) \leq p_*(\pi_1(E, e_0))\]
\end{thm}

\begin{proof}
    ($\Rightarrow$) Once the lifting $\tilde{f}$ exists, then $p \circ \tilde{f} = f$. 
    By the functorial property, $p_* \circ \tilde{f}_* = f_*$. 
    Hence, $\Im(f_*) = \Im(p_* \circ f_*) \leq \Im(p_*)$. 

    ($\Leftarrow$) We'll construct $\tilde{f}: Y \rightarrow E$ such that well-defined, continuous 
    and $p \circ \tilde{f} = f$. 
    For any given $y \in Y$, since $Y$ is path-connected, we can find $\alpha$ a path from $y_0$ to $y$. 
    Send $\alpha$ via continuous function $f$, so that $f \circ \alpha$ is path in $B$ from $b_0$ to $f(y)$. 
    By fix the initial point be $e_0$, we can lift $f \circ \alpha$ into $E$, 
    \[\widetilde{f \circ \alpha} \text{ is a path from $e_0$ to somewhere in $E$ }\]
    Now we define 
    \[\tilde{f} : y \mapsto \widetilde{f \circ \alpha}(1)\]
    This is well-defined i.e., independent to the choice of path from $y_0$ to $y$. 
    Suppose that we choose another path $\beta$ from $y_0$ to $y$. 
    Then, $\alpha * \overbar{\beta}$ forms a loop in $Y$ based at $y_0$. 
    Hence, 
    \[[f \circ (\alpha * \overbar{\beta})] \in \Im(f_*) \leq \Im(p_*)\]
    So we can find $[\gamma] \in \pi_1(E, e_0)$ such that 
    \[[f \circ (\alpha * \overbar{\beta})] = [p \circ \gamma] \implies f \circ (\alpha * \overbar{\beta}) \simeq_p p \circ \gamma\]
    By Homotopy Lifting Theorem \ref{thm:homotopy lifting theorem}, we obtain lifting of $f \circ (\alpha * \overbar{\beta})$ and $\gamma$ are homotopic. 
    Hence $f \circ (\alpha * \overbar{\beta})$ is a loop based at $e_0$, implies that $f \circ \alpha(1) = f \circ \overbar{\beta}(0) = f \circ \beta(1)$. 
    So that $\tilde{f}$ is well-defined: 
    \[\widetilde{f \circ \alpha}(1) = \widetilde{f \circ \beta}(1)\]
    Second, following holds. 
    \[p \circ \tilde{f}(y) = p \circ \tilde{f}(\alpha(1)) = p \circ \widetilde{f \circ \alpha}(1) = f \circ \alpha(1) = f(y)\]
    Hence, 
    \[p \circ \tilde{f} = f\]
    Lastly, we'll show that $\tilde{f}: Y \rightarrow E$ is continuous map. 
    We've shown that in \ref{thm:thm_constructing continuous functions}, $Y$ is continuous 
    when it is partially continuous on each open covering elements. 
    Let $y \in Y$ and we can find $f(y) \in B$. It has $f(y) \in U$ where $U$ is evenly covered by $p$. 
    Then we can find sheets of $U$ in $E$, denoted by $\{V_\alpha\}$. 
    There is unique sheet $V_*$ where $\tilde{y} \in V_*$. Note that $p|_{V_*} : V_* \rightarrow U$ is 
    homeomorphism. For $f^{-1}(U)$ open set in $Y$, we can find locally path connected open set $W$ that $y \in W \subseteq U$. 
    Then $\tilde{f}|_W$ is path-connected in $E$ hence $\tilde{f}|_W \subseteq V$. Hence 
    $\tilde{f}|_W, p|_V$ are homeomorphism, so $\tilde{f}|_W$ is continuous, implies that $\tilde{f}$ is continuos.
\end{proof}
...
\begin{proof}
    Since $\tilde{B}$ is simply connected, it is path-connected locally path-connected, and $\pi_1(\tilde{B}) = 1$. 
    Hence for any covering map $q$, we obtain that $\Im(p_*) = 1 \leq \Im(q_*)$ hence by \ref{thm:lifting criterion}, 
    there exists unique lifting of $p$, denoted by $r: (\tilde{B}, \tilde{b}_0) \rightarrow (E, e_0)$ so $q \circ r = p$. 

    Now we're showing that such $r$ is indeed covering map. We already obtain that $r$ is continuous, 
    hence it is sufficient to show that (1) $r$ is surjective (2) $r$ satisfies the evenly covered condition. 

    (1) Choose arbitrary $e \in E$ then as $E$ is path-connected, we can find a path $\alpha$ from $e_0$ to $e$. 
    We take it into $B$ through $q$, denoted by $q \circ \alpha$ which is a path from $b_0$ to $b = q(e)$. 
    It has unique lifting when we fix the initial point of lifting path by $\tilde{b}_0$. 
    Denote the lifting by $\widetilde{q \circ \alpha}$ is the path from $\tilde{b}_0$ to $\tilde{b}$ where $p(\tilde{b}) = q(e)$. 
    By the commuteness, $r \circ \widetilde{q \circ \alpha} = \alpha$ hence $r \circ \widetilde{q \circ \alpha}(1) = r(\tilde{b}) = \alpha(1) = e$
    So $r$ is surjective. 

    (2) Choose arbitrary $e \in E$. For $b = f(e) \in B$, we can find evenly covered neighborhood $U$ such that 
    evenly covered by both $p, q$ covering maps. 
    Hence, we can find homeomorphic sheets $\{\tilde{V}_\alpha\}$ over $\tilde{B}$ 
    and $\{U_\beta\}$ over $E$. 
    Since they are disjoint homeomorphic to each others and commuteness implies that 
    there are a surjection from $\{\tilde{V}_\alpha\}$ to $\{U_\beta\}$. 
    Then for $e \in U_*$, we can obtain its preimage over $r$ is subset of $\{\tilde{V}_\alpha\}$ 
    hence $U_*$ is evenly covered by $r$. So the lifting of $p$, $r$ is a covering map. 
\end{proof}
...
One false-example of semi-locally simply connected is Hawaiian Earring defined by 
radius $1/n$ circles intersects unique point $0$ in $\BR^2$. At $0$, any neighborhood of $0$ contains 
some circles in there, loop on them does not contractable into origin even entire space.
By the way, cone on Hawaiian Earring is semi-locally simply connected and extract top of the cone is again false example. 
